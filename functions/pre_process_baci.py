...
def pre_process_data(path):
    ...
    r_cc1 = json.loads(
        requests
        .get('https://comtrade.un.org/data/cache/reporterAreas.json')
        .content
        .decode('utf-8-sig'))    
    r_cc2 = json.loads(
        requests
        .get('https://comtrade.un.org/data/cache/partnerAreas.json')
        .content
        .decode('utf-8-sig'))
    #product code
    ...
    print('Start replacing faulty country names...')
    ...
    for year in tqdm(data_dictio.keys()):
        data_dictio[year].replace(revised_country_names, inplace=True)
        data_dictio[year].replace(revised_country_names, inplace=True)
    
    print('Replacing faulty country names completes...')
